  ...
  try {
    ...
    for (let [key, value] of Object.entries({ fname, lname })) {
      ...
      if (!valid.isValidName(value)) {
        return res.status(400).send({ status: false, msg: ` ${key} should be in string and alphabets only..`});
      }
    }
    ...
    if (!valid.isValidEmail(email)) {
      return res.status(400).send({ status: false, msg: "email should be string and follow basic email format."});
    }
    ...
    if (!valid.isValidMobile(phone)) {
      return res.status(400).send({ status: false, msg: "Please provide valid indian mobile number."});
    }
    ...
    if (!address || typeof address !== "object") {
      return res.status(400).send({ status: false, msg: "Please provide address in object format." });
    }
    reqBody.address = address;

    let { shipping, billing } = reqBody.address;
    if (!shipping || !billing) {
      return res.status(400).send({ status: false, msg: "shipping and billing addresses are required." });
    }
...
const loginUser = async function (req, res) {
  try {
    ...
    res.status(200).send({ status: true, message: "Successfully logged in.", data: { userId: emailExist._id, token }});
  } catch (err) {
...
const userProfileDetails = async function (req, res) {
  ...
  return res.status(200).send({ status: true, msg: "User profile details.", data: getUserDetails });
}catch(err){
  return res.status(500).send({ status: false, msg: err.message })
}
};
...
const updateUserDetails = async function (req, res) {
  try {
    ...
    if (!hasBody && !hasFiles) {
      return res.status(400).send({ status: false, msg: "Please provide user details which you want to update.." });
    }
    let { fname, lname, email, phone, password, address } = reqBody;
    ...
    if (fname !== undefined) {
      ...
      if (!valid.isValidName(fname)) {
        return res.status(400).send({ status: false, msg: " fname should be in string and alphabets only.."});
      }
      reqBody.fname = fname
    }
    ...
    if (lname !== undefined) {
      ...
      if (!valid.isValidName(lname)) {
        return res.status(400).send({ status: false, msg: " last name should be in string and alphabets only.."});
      }
      reqBody.lname = lname
    }
    if (email !== undefined) {
      ...
      let emailExist = await userModel.findOne({ email, _id: { $ne: useIdFromReq } });
      if (emailExist) {
        return res.status(409).send({ status: false, msg: "user email already exist.." });
      }
      reqBody.email = email
    ...
    if (phone !== undefined) {
      ...
      if (!valid.isValidMobile(phone)) {
        return res.status(400).send({ status: false, msg: "Phone number should be 10 digit and valid indian number.."});
      }
      let phoneNoExist = await userModel.findOne({ phone, _id: { $ne: useIdFromReq } });
      if (phoneNoExist) {
        return res.status(409).send({ status: false, msg: "Mobile number already exist.." });
      }
      reqBody.phone = phone
